...
verification on pull requests and pushes to `main`. The parity test also
injects a stale generated file and proves the check fails. GitHub Packages
publishing is restricted to a stable (non-prerelease) GitHub release. The
workflow checks out the release tag, installs with the frozen lockfile, verifies
that the tag is `v` plus the package version, and publishes with the workflow
token. The package's `prepublishOnly` hook runs `verify` once before publication.
