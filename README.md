...
frozen Bun lockfile, verifies the tag's package version, and publishes with the
workflow token to GitHub Packages. The published package ships `dist`, so
...
committed CI workflow installs with the frozen Bun lockfile and runs `verify` on
pull requests and pushes to `main`. Publishing occurs only from a stable GitHub
release after the workflow verifies the tag's package version; publication then
runs the package's `prepublishOnly` check, which invokes `verify` once before
publishing.
